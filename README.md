...
| イベント通知 | Web hubで施設名/ID・時間をGLABへ登録し、Discordへ定期リマインド（GLAB PostgreSQL共有） |
...
│   ├── commands/  # 空のSlash登録と、同期前に残るcommandへの停止案内
│   ├── notify/    # GLABからDiscordへの通知
...
既存 Slash command の削除は、過去に使った guild / global の両 scope で空配列を登録する必要がある。
手順は [`spec/interface/discord-commands.md`](./spec/interface/discord-commands.md) を参照。
