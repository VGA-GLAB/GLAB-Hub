...
const membersModule: CorpusModule = {
  id: 'members',
  title: '部員名簿',
  icon: '📋',
  setup(ctx: CorpusContext) {
      ...
      async function resolveName(row: MemberRow): Promise<string> {
        ...
        try {
          const profile = await getVantanUserProfile(cernere, row.user_id);
      ...
      router.post('/:id/link', requireAdmin, async (c) => {
        ...
        if (!row) return c.json({ error: 'invalid_or_already_linked_user' }, 409);
        return c.json({ ok: true, member: await view(row) });
      });
      ...
      router.get('/link-candidates', requireAdmin, async (c) => {
        ...
        const withNames = await Promise.all(candidates.map(async (u) => ({
          userId: u.user_id,
          name: cernere
            ? await getVantanUserProfile(cernere, u.user_id).then((p) => p.name || '(未登録プロフィール)').catch(() => '(取得失敗)')
            : '(Cernere 未接続)',
        })));
