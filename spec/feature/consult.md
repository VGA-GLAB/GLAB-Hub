...
旧 `/orehima` handler は到達不能な移行用ソースとして残るが、registry への登録と dispatch はない。

**未解決（要決着）**: Corpus は `/api/*` 全体に `requireAuth`（Cernere accessToken 検証）を掛けており、service token はその **内側** の追加ゲートでしかない（projects と同じ制約）。bot は現状 Cernere の bearer を持たないため、service token だけの呼び出しは `401 unauthorized` で弾かれる。bot 用の Cernere 資格情報を配るか、hub 側に service token 単独で通せる到達経路を用意するまで、相談の Discord 通知は degraded のまま動かない。
